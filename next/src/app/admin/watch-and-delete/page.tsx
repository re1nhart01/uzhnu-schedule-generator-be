...
import { DateDeleteForm } from "@/components/date-delete-form/date-delete-form";

export default async function ScheduleAdminPage({
  searchParams,
}: {
  searchParams: { dates?: string };
}) {
      ...
      <div className="space-y-8 px-4 sm:px-6 lg:px-8 py-6 w-full h-full flex-1">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-row gap-4">
              <DateDeleteForm availableDates={fetchedDates?.map((date) => date.created_at) ?? []} selectedDates={dates ?? []} />
          </div>
        </div>
